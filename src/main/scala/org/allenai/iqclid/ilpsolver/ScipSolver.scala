...
/** This is a generic interface to the SCIP ILP solver providing a number of common initialization
  ...
  */
class ScipSolver(probName: String, scipParams: ScipParams, calculateGradients: Boolean)
    extends IlpSolverWithObjGradients[ScipVar, ScipCons](calculateGradients) with Logging {
  ...
  // create the SCIP quadratic constraint environment
  private val envConsQuad = new JniScipConsQuadratic
  ...
  def addConsQuadratic(name: String, linearVars: Seq[ScipVar], linearCoeffs: Seq[Double],
                       quadVars1: Seq[ScipVar], quadVars2: Seq[ScipVar], quadCoeffs: Seq[Double],
                       lhsOpt: Option[Double], rhsOpt: Option[Double]): Unit = {
    addReleaseCons(createConsQuadratic(name, linearVars, linearCoeffs, quadVars1,
      quadVars2, quadCoeffs, lhsOpt, rhsOpt))
  }

  private def createConsQuadratic(name: String, linearVars: Seq[ScipVar], linearCoeffs: Seq[Double],
                                  quadVars1: Seq[ScipVar], quadVars2: Seq[ScipVar], quadCoeffs: Seq[Double],
                                    lhsOpt: Option[Double], rhsOpt: Option[Double]): ScipCons = {
    ScipCons(
      envConsQuad.createConsBasicQuadratic(scip, name, linearVars.length, linearVars.map(_.ptr).toArray,
      linearCoeffs.toArray, quadVars1.length, quadVars1.map(_.ptr).toArray,
      quadVars2.map(_.ptr).toArray, quadCoeffs.toArray, lhsOpt.getOrElse(ScipMin), rhsOpt.getOrElse(ScipMax))
    )
  }

  //public native long createConsQuadratic(long var1, String var3, int var4, long[] var5, double[] var6, int var7, long[] var8, long[] var9, double[] var10, double var11, double var13, boolean var15, boolean var16, boolean var17, boolean var18, boolean var19, boolean var20, boolean var21, boolean var22, boolean var23) throws NativeScipException;
